...
def get_materials_parallel_denoise(accelerate, stable_material, save_path, input_image, pose, mask, args, num_inference_loops=1, num_inference_steps=50, weight_dtype=torch.float16, modifier=""):
    ...
    if os.path.exists(albedo_path):
        ...
        with torch.no_grad():
            prompt_embeds = stable_material._encode_image_with_pose(input_image, pose, accelerate.device, 1, args.sm_guidance_scale > 1.0) # DINO
...
def get_relightings_parallel_denoise(accelerate, pipeline, input_image, pred_albedo, pred_orm, envs_darker_target, envs_brighter_target, dir_embeds, pluckers, args, generator=None, num_inference_steps=50, num_inference_loops=1):
    ...
    scene_features = pipeline.encode_env(envs_darker_target, envs_brighter_target, dir_embeds, do_classifier_free_guidance, accelerate.device, input_image.dtype) # DINO
...
def produce_colmap(accelerate, args, data_path, pipeline, stable_material, weight_dtype=torch.float16, generator=None): # 函数名很misleading
    ...
    mask = batch["mask"]#.to(dtype=weight_dtype) # 其实这里我跑高斯的时候重新手动resize了一下mask然后注册到高斯，他要refactor来的，这一部分我可以给他代码补全
    ...
    pred_albedo, pred_orm = get_materials_parallel_denoise(accelerate, stable_material, os.path.split(results_dir)[0], input_image, pose, mask, args, num_inference_steps=35, num_inference_loops=1) # stable_material: StableMaterialPipelineMV
    temp_relit_images_ours = get_relightings_parallel_denoise(accelerate, pipeline, input_image, pred_albedo, pred_orm, envs_darker_target, envs_brighter_target, dir_embeds, pluckers, args, generator=generator, num_inference_steps=35, num_inference_loops=1) # pipeline: RelightingPipelineMVVAE 在这里加入envmap（和input image组成pair）
...
def produce_gs_relightings(args, weight_dtype=torch.float16):
    ...
    ) # args.pretrained_model = thebluser/lightswitch-multi-fov, 只改动了condition UNet2DConditionModel
    
    # 传入dino encoder
    REPO_DIR = "/root/autodl-tmp/dinov3/dinov3"
    WEIGHT_PATH = "/root/autodl-tmp/.cache/torch/hub/checkpoints/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth"
    MODEL_NAME = "dinov3_vitb16"
    dino_encoder = torch.hub.load(REPO_DIR, MODEL_NAME, source='local', weights=WEIGHT_PATH)
    dino_encoder.half().cuda()
    ...
    stable_material = StableMaterialPipelineMV.from_pretrained(args.pretrained_model_sm, torch_dtype=weight_dtype, trust_remote_code=True) # args.pretrained_model_sm = thebluser/stable-material-mv
    if args.torch_compile:
        pipeline.unet = torch.compile(pipeline.unet, mode="reduce-overhead", fullgraph=True)
        stable_material.unet = torch.compile(stable_material.unet, mode="reduce-overhead", fullgraph=True)

    if args.enable_xformers_memory_efficient_attention:
        if True: # is_xformers_available():
            import xformers
...
if __name__ == "__main__":
    ...
    dtype = torch.float16
    produce_gs_relightings(args, dtype) # pipeline: RelightingPipelineMVVAE