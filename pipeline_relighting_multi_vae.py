    ...
    def prepare_condition_latents(self, image, albedo, orm, pluckers, batch_size, dtype, device, generator=None, do_classifier_free_guidance=False):
        ...
        if isinstance(generator, list):
            ...
            if self.dino_encoder is not None:
                dino_feats = self.dino_encoder(image)  # [B, 768]
                
                unet_in_channels = 22  
                latent_channels = init_img_latents.shape[1]
                proj_channels = unet_in_channels - latent_channels  
          
                proj = nn.Linear(768, proj_channels).cuda().half()
                dino_feats_proj = proj(dino_feats)              # [B, proj_channels]
                dino_feats_proj = dino_feats_proj[:, :, None, None]  # [B, proj_channels, 1, 1]
                dino_feats_proj = dino_feats_proj.expand(-1, -1, init_img_latents.shape[2], init_img_latents.shape[3])
